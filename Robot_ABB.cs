...
namespace AutoFrame
{
    ...
    class Robot_ABB
    {
        ...
        private static Controller[] controller=new Controller[3];
        ...
        public static void Init_All_Robot()
        {
            ...
            {
                if (info.ControllerName == "CF_Peel")
                {
                    if (info.Availability == Availability.Available)
                    {
                        if (controller[(int)controller_name.Peel] != null)
                        {
                            controller[(int)controller_name.Peel].Logoff();
                            controller[(int)controller_name.Peel].Dispose();
                            controller[(int)controller_name.Peel] = null;
                        }
                        controller[(int)controller_name.Peel] = ControllerFactory.CreateFrom(info);
                       
                    }
                }

                if (info.ControllerName == "CF_Load")
                {
                    if (info.Availability == Availability.Available)
                    {
                ...
                if (info.ControllerName == "CF_Assemble")
                {
                    if (info.Availability == Availability.Available)
                    {
                        if (controller[(int)controller_name.Assemble] != null)
                        {
                            controller[(int)controller_name.Assemble].Logoff();
                            controller[(int)controller_name.Assemble].Dispose();
                            controller[(int)controller_name.Assemble] = null;
                        }
                        controller[(int)controller_name.Assemble] = ControllerFactory.CreateFrom(info);
                    }
                }
            }
